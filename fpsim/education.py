...
class Education:
    ...
    def initialize(self, filteredppl):
        education_dict = self.pars

        f = filteredppl.filter("female")
        f_urban, f_rural = f.filter("urban", split=True)
        ...
        f_rural.edu_objective = np.random.choice(edu_years, size=len(f_rural), p=probs_rural)
        f_urban.edu_objective = np.random.choice(edu_years, size=len(f_urban), p=probs_urban)

        f_ages = np.floor(f.age).astype(int)
        f.edu_attainment = np.floor((education_dict['edu_attainment'][f_ages]))
        started = f.filter(f.edu_attainment > 0.0)
        completed = f.filter(f.edu_objective-f.edu_attainment <= 0.0)

        completed.edu_attainment = completed.edu_objective
        completed.edu_completed = True
        started.edu_started=True
    ...
    def update(self, filteredppl):
        self.start_education(filteredppl)
        self.advance_education(filteredppl)
        self.resume_education(filteredppl)
        self.graduate(filteredppl)
    ...
    def interrupt_education(ppl):
        pregnant_students = ppl.filter(ppl.pregnant & (ppl.gestation == ppl.sim.fp_pars['end_first_tri']))
    ...
    def advance_education(self, ppl):
        ...
        students.edu_attainment = students.edu_attainment + ppl.people.sim.t.dt_year
    ...
    def resume_education(ppl):
        ...
        filter_conds = (ppl.postpartum & ppl.edu_interrupted & ~ppl.edu_completed & ~ppl.edu_dropout &
                        (ppl.postpartum_dur > 0.5 * ppl.people.sim.fp_pars['postpartum_dur']))
    ...
    @staticmethod
    def graduate(ppl):
        completed = ppl.filter(ppl.edu_attainment >= ppl.edu_objective)
        completed.edu_completed = True
    # @staticmethod
