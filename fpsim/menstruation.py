...
class contra_hmb(ss.Intervention):
    ...
    def step(self):
        if self.sim.t.now() == self.pars.year:
            ...
            self.sim.people.method[accept_uids] = self.iud_idx
            self.sim.people.on_contra[accept_uids] = True
            self.sim.people.ever_used_contra[accept_uids] = True
...
if __name__ == '__main__':
    ...
    sim = fp.Sim(location='kenya', interventions=contra_hmb, start=2020, stop=2030)
