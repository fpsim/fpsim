...
if __name__ == '__main__':
    ...
    figs_dir = os.path.abspath(os.path.join(thisdir, '..', country, 'regions', 'figs', region))
    ...
    data_asfr = pd.read_csv(f'../{country}/regions/data/asfr_region.csv').loc[lambda df: df['region'] == region]
    data_methods = pd.read_csv(f'../{country}/regions/data/mix_region.csv').loc[lambda df: (df['region'] == region) & (df['year'] == pars['end_year'])]
    data_tfr = pd.read_csv(f'../{country}/regions/data/tfr_region.csv').loc[lambda df: df['region'] == region]
    data_use = pd.read_csv(f'../{country}/regions/data/use_region.csv').loc[lambda df: (df['region'] == region) & (df['year'] == pars['end_year'])]
    ...
    if do_plot_cpr:
        ...
        if do_save:
            pl.savefig(f'{figs_dir}//cpr_over_sim.png')
    ...
    if do_plot_tfr:
        ...
        if do_save:
            pl.savefig(f'{figs_dir}//tfr_over_sim.png')
    ...
    if do_plot_pop_growth:
        ...
        if do_save:
            pl.savefig(f'{figs_dir}/popgrowth_over_sim.png')
    ...
    if do_plot_birth_space_afb:
        ...
        if do_save:
            pl.savefig(f'{figs_dir}/birth_space_bins.png', bbox_inches='tight', dpi=100)
