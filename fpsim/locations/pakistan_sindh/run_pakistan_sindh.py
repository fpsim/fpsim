"""
Script to create a calibrated model of Pakistan Sindh.
...
# Settings
country = 'pakistan_sindh'    # <<< USER-EDITABLE 
...
def make_pars():
    ...
    pars['fecundity_var_low'] = 0.1 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py   
    pars['fecundity_var_high'] = .9 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py   
    pars['exposure_factor'] = 4 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py  
    ...
    cm_pars = dict(prob_use_year = 2020,  # Base year
                   ...
                   prob_use_intercept = -2.7,  # Intercept for the probability of using contraception - shifts the mCPR level
                   method_weights = np.array([0.06, 2.3, 20, 2, 2.5, 1.5, 3, 0.1, 0.01]))
    ...
    pars['spacing_pref']['preference'][6:9] = 5  # Spacing of 18-24 months
    pars['spacing_pref']['preference'][9:] =  0.1  # Spacing of 27-36 months
