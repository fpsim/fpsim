"""
This configuration file is for an FPsim model specific to Pakistan Sindh.
...
def exposure_age():
    ...
    exposure_correction_age = np.array([[0,     5, 10, 12.5, 15, 18, 20, 25, 30, 35, 40, 45, 50],
                                        [0.2, 0.2, 0.2,  1 ,0.9, 0.8,0.8,1.25,1, 0.7,0.5,0.5, 0.5]])  # <<< USER-EDITABLE: Can be modified for calibration
...
def make_pars(location='nigeria_lagos', seed=None):  # <<< USER-EDITABLE: Change name of location; country name if country, region name if region
    """
