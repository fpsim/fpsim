"""
Template script to create a calibrated model of a specific Country or Region.
...
# Settings
country = 'country{_region}'    # <<< USER-EDITABLE   
...
def make_pars(country):
    ...
    pars['fecundity_var_low'] = 1 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py 
    pars['fecundity_var_high'] = 1 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py  
    pars['exposure_factor'] = 1 # <<< USER-EDITABLE: Adjust/override any parameters that are defined in fpsim/defaults.py  
    ...
    cm_pars = dict(prob_use_year = 2020,  # Base year 
                   prob_use_trend_par = 1,  # Time trend in contraceptive use - adjust this to get steeper/slower trend
                   prob_use_intercept = 1,  # Intercept for the probability of using contraception - shifts the mCPR level
                   method_weights = np.array([1, 1, 1, 1, 1, 1, 1, 1, 1]))
