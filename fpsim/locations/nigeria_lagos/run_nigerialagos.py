...
def make_pars():
    ...
    pars['fecundity_var_low'] = 2.2
    pars['fecundity_var_high'] = 0.7
    pars['exposure_factor'] = 0.5
    
    # Adjust contraceptive choice parameters
    cm_pars = dict(prob_use_year = 2020,  # Base year
                   prob_use_trend_par = 0.05,  # Time trend in contraceptive use - adjust this to get steeper/slower trend
                   prob_use_intercept = -2.1,  # Intercept for the probability of using contraception - shifts the mCPR level
                   method_weights = np.array([1.9, 3, 6, 0.08, 0.1, 0.001, 3, 0.03, 0.7]))
    ...
    pars['spacing_pref']['preference'][6:9] = 0.6  # Spacing of 18-24 months
    pars['spacing_pref']['preference'][9:] =  0.5  # Spacing of 27-54 months
