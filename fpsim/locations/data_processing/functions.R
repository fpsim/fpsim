...
new.data = data.frame(expand.grid(age_grp_2 = unique(filter_data$age_grp_2), fp_ever_user_2 = unique(filter_data$fp_ever_user_2))) %>% 
  filter (!is.na(age_grp_2) & !is.na(fp_ever_user_2)) 
predicted.p <- predict(model.simple, newdata = new.data, type = "response")
...
lasso.vars <- c("intent_cat_1", "paidw_12m_1", 
                ...
                "fp_ever_user_2", "age_2", "yrs.edu_2", "live_births_2", "urban_2", "wealthquintile_2")
...
design_matrix <- model.matrix(~ (intent_cat_1 + paidw_12m_1 + 
                                   ...
                                   savings_1 + financial_info_1 + financial_goals_1 +
                                   decision.making_1 + financial.autonomy_1 +
                                   fp_ever_user_2 + ns(age_2, knots = c(25,40)) + yrs.edu_2 + live_births_2 + urban_2 + wealthquintile_2)^2, 
...
contra_coef <- data.frame(
  ...
  Estimate = lasso_coefs[as.vector(lasso_coefs) != 0]) %>% # switch to comma if using next row
