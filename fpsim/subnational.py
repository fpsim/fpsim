...
def initialize_lam_region(self, n, lam_region):
    ...
    for res in lam_by_region['region'].unique():
        # Find indices in region array
        f_inds = sc.findinds(lam_region == res)
        for month in lam_by_region[lam_by_region['region'] == res]['month'].unique(): 
            month_data = lam_by_region[(lam_by_region['region'] == res) & (lam_by_region['month'] == month)]
